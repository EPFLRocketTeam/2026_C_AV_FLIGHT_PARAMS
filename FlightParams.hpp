...
struct PressurizationParams {
    FIXED   uint32_t LaunchDelayMs = 30.f SECONDS;
    DYNAMIC uint32_t HoldDelayMs   = 23.8f SECONDS;
    ...
    DYNAMIC float StableBVOpeningLox  = 45;
    DYNAMIC float StableBVOpeningFuel = 45;
    ...
    inline float SafetyLoxBBDPRCloseThreshold  () const { return SafetyBBDPRCloseThresholdRatio * TargetPressureLox; }
    inline float SafetyLoxBBDPROpenThreshold   () const { return SafetyBBDPROpenThresholdRatio  * TargetPressureLox; }
...
static_assert(sizeof(PressurizationParams) == 52);

struct IgnitionParams {
    DYNAMIC uint32_t PrechillDurationMs = 200;
    DYNAMIC uint32_t IgniterDurationMs  = 6000;
    DYNAMIC uint32_t DelayMs            = 108;
...
#define PRINT_FLIGHT_PARAMS(params) { \
    ...
    printf("config pressurize stable_opening_lox %f\n",        params.Pressurization.StableBVOpeningLox); \
    printf("config pressurize stable_opening_fuel %f\n",       params.Pressurization.StableBVOpeningFuel); \
    printf("config pressurize preburn_duration_lox %f\n",      params.Pressurization.PreburnDurationLoxMs); \
    printf("config pressurize preburn_duration_fuel %f\n",     params.Pressurization.PreburnDurationFuelMs); \
